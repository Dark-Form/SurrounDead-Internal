#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BPI_Damage

#include "Basic.hpp"

#include "BPI_Damage_classes.hpp"
#include "BPI_Damage_parameters.hpp"


namespace SDK
{

// Function BPI_Damage.BPI_Damage_C.Damage_Object
// (Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// double                                  Damage                                                 (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class AActor*                           Damage_Causer                                          (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)
// class AController*                      Event_Instigator                                       (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)

void IBPI_Damage_C::Damage_Object(double Damage, class AActor* Damage_Causer, class AController* Event_Instigator)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = AsUObject()->Class->GetFunction("BPI_Damage_C", "Damage_Object");

	Params::BPI_Damage_C_Damage_Object Parms{};

	Parms.Damage = Damage;
	Parms.Damage_Causer = Damage_Causer;
	Parms.Event_Instigator = Event_Instigator;

	AsUObject()->ProcessEvent(Func, &Parms);
}


// Function BPI_Damage.BPI_Damage_C.Damage_Shoved
// (Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool                                    Anim                                                   (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// double                                  Force                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void IBPI_Damage_C::Damage_Shoved(bool Anim, double Force)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = AsUObject()->Class->GetFunction("BPI_Damage_C", "Damage_Shoved");

	Params::BPI_Damage_C_Damage_Shoved Parms{};

	Parms.Anim = Anim;
	Parms.Force = Force;

	AsUObject()->ProcessEvent(Func, &Parms);
}


// Function BPI_Damage.BPI_Damage_C.IsObjectDamageable?
// (Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool*                                   Damageable_                                            (Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void IBPI_Damage_C::IsObjectDamageable_(bool* Damageable_)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = AsUObject()->Class->GetFunction("BPI_Damage_C", "IsObjectDamageable?");

	Params::BPI_Damage_C_IsObjectDamageable_ Parms{};

	AsUObject()->ProcessEvent(Func, &Parms);

	if (Damageable_ != nullptr)
		*Damageable_ = Parms.Damageable_;
}

}

