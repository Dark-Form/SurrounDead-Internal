				...
				if (ImGui::BeginTabBar("MainTabs", ImGuiTabBarFlags_FittingPolicyScroll))
				{
					...
					if (ImGui::BeginTabItem("Waypoints"))
					{
						ImGui::BeginGroup();
						{
							...
							int currentIcon = static_cast<int>(WaypointSystem::WaypointManager::currentIcon);
							if (ImGui::Combo("Waypoint Icon", &currentIcon, iconNames, IM_ARRAYSIZE(iconNames))) {
								WaypointSystem::WaypointManager::currentIcon = static_cast<WaypointSystem::WaypointIcon>(currentIcon);
							}
							...
							if (WaypointSystem::WaypointManager::selectedWaypoint >= 0 && 
								...
								if (ImGui::Combo("##EditIcon", &currentIcon, iconNames, IM_ARRAYSIZE(iconNames))) {
									WaypointSystem::WaypointManager::waypoints[WaypointSystem::WaypointManager::selectedWaypoint].icon = 
										static_cast<WaypointSystem::WaypointIcon>(currentIcon);
									WaypointSystem::WaypointManager::SaveWaypoints();
								}
